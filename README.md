...
In the examples below, replace `user_name@ip_address` with your Raspberry Pi
login target, or use your own SSH alias if you have one configured.

Copy both artifacts to the Pi:

```sh
scp build/kernel_module/stepper_hat.ko build/stepperctl/stepperctl user_name@ip_address:/tmp/
```

Install the CLI on the Pi:

```sh
ssh user_name@ip_address 'sudo install -m 0755 /tmp/stepperctl /usr/local/bin/stepperctl'
```

Load the driver:

```sh
ssh user_name@ip_address 'sudo insmod /tmp/stepper_hat.ko'
```
...
```sh
ssh user_name@ip_address 'dmesg | tail -n 20'
```
...
```sh
ssh user_name@ip_address "echo 'KERNEL==\"stepper_hat\", GROUP=\"gpio\", MODE=\"0660\"' | sudo tee /etc/udev/rules.d/99-stepper-hat.rules >/dev/null"
ssh user_name@ip_address 'sudo udevadm control --reload-rules && sudo udevadm trigger'
ssh user_name@ip_address 'sudo rmmod stepper_hat && sudo insmod /tmp/stepper_hat.ko'
```
